...
# Note: This is a MODIFIED version of the session.pm found in other places.
# Mods: 
# 	1) Cookie 'user' is looked up to get the session id if it is not sent in. This is always used if it exists rather than just generating a new session id.
# 	This is more logical than having to look up the session ID outside of the module and pass it in.
...
# Function: new (session_id (optional))
# Param session_id = identifier of session; if not sent the cookie is looked up. If no session is not found, a new ID is created
# Returns: $session object
# Meta: all new sessions have $session->{ID} prepopulated
sub new($$$) {
...
# Meta: Saves session hash up to 4 levels deep
sub Save_recursive($$$) {
    my ($fh, $var, $prefix) = @_;
...
# Param: session_id = identification string of the session
...
# Function: Decrypt_Session_string ($input)
# Param: input = encrypted session string
# Returns: decrypted session string
# Meta: Session strings are encrypted by replacing each
...
# Function: Encrypt_Session_string ($input)
...
# Function: Clear_Files_Session ()
# Meta: Scans all files in SESSIONS_FOLDER. If a
...
# Meta: returns: 1 if input string contains only
