...
#-------------------------------------------------------------------------------
#only housseine gets admin enabled.(sep6th, 2013)
#-------------------------------------------------------------------------------
	if ($login eq 'housseine.rejouan'){
		$resp{'admin'} = 'true';
	} else {
		$resp{'admin'} = 'false';	
	}
...
sub GetbDistinguishedNameFromUserName($$) {
	...
	my $mesg = $ldap->bind;
	$mesg = $ldap->search(base => "[Add base here]",
...
sub usage {
	...
	use BC_LDAP qw(ldap_authenticate ldapSafeName  GetbDistinguishedNameFromUserName);

	and call the main function using:

	BC_LDAP::ldap_authenticate(\$username,\$password);
EOT
}
