			...
			if (data.errorMessage){
				...
				if (url.match(/get_entitlement/)){
					...
					if (Object.keys(data.content.Entitlements).length>0){
						entitlementData = "<p style='background-color:#444; color:#eee; padding-left:5px; border-radius:5px; padding-top:3px; padding-bottom:3px;'><b>Bundles:</b></p>";
					}
					for (x in data.content.Entitlements){
						...
						for (y in data.content.Entitlements[x]){	
							if (y == "bundleDescription" || y == "device"){ continue;}
							var print_y = y.camelCaseToRegular();
							entitlementData = entitlementData + print_y + " - " + data.content.Entitlements[x][y] + "\n";
						}
					}

					var specData="";
					if (Object.keys(data.content.Specs).length>0){
						var specData = "<p style='background-color:#444; color:#eee; padding-left:5px; border-radius:5px; padding-top:3px; padding-bottom:3px;'><b>Subscriptions:</b></p>";
					}
					for (x in data.content.Specs){
						var print_x = x.camelCaseToRegular().replace(/_/g, " ");
						specData = specData + "<p style='font-weight:bold'>" + print_x + "</p>";
						for (y in data.content.Specs[x]){
							if (y == "featureName"){ continue;}
							var print_y = y.camelCaseToRegular();
				...
				// Remove component
					...
					console.log(data);
					alertBox("Component removed successfully - please reload this tab to get updated data from the server.");
					...
					if (data.success){
						...
						console.log(data);
						alertBox("Error in syncing - please check the console","ext-mb-error");
					}
				// add note
				} else if (url.match(/add_note/)){
						alertBox("Note updated successfully.","mb-ext-ok");
...
function format_asset_data(data,el){
	...
	if (data.content.Asset['SerialNumber']==undefined){
		output = "<p class='errormsg'>No asset data found for serial number " + document.forms['searchForm'].elements['search'].value + ".</p>";
	} else {
		...
		output += ' <button id="note_' + data.content.Asset['SerialNumber'] + '" class="add_note" title="Add a note to yourself about this appliance" onClick="addNote(\'' + data.content.Asset['SerialNumber'] + '\')">[ Add note ]</button>';
		...
		if (data.content.Asset['EndUserAccountName']){
			output += '<br />Account: <span class="ui-li-aside">' +  data.content.Asset['EndUserAccountName'] + '</span><br />';
			if (!data.content.Asset['SiteAddress']){ data.content.Asset['SiteAddress']="(not available)"}
			output += 'Site Address: <span class="ui-li-aside">' +  data.content.Asset['SiteAddress'] + '</span><br />';
		}
 ...
 * Meta: Read the components json data and format it for the portal 
*/
...
function showFullAssetData(clickElement,serial){
	...
	var myElement = "assetData_"+serial;
	if (document.getElementById(myElement).style.display=="block"){
		$(clickElement).html('[+] Show all Asset Data');
		$('#assetData_' + serial).fadeOut();
	} else {
		$('#assetData_' + serial).fadeIn();
		$('#AssetIntro_' + serial).hide();
		$(clickElement).html('[-] Hide full Asset Data');
		$(clickElement).hide();
	}
...
function addComponent(applianceSerial){
	...
	function promptResponse(btn,component_serial) {
		...
		if (btn=="ok"){
			...
			alertBox("No new component was added.","ext-mb-error");
		}
...
function searchMultiple(){
	...
	loopThroughAppliances(goodSnList);

}

function loopThroughAppliances(splitText) {
...
function checkComponent(){
	...
	if (!value){ alertBox("No serial entered","ext-mb-error"); }
...
function checkComponentByActivationCode(){
	...
	if (!value){ alertBox("No code entered","ext-mb-error"); }
	...
	$.ajax({
		...
		complete: function(jqXHR, textStatus){
			...
			switch (jqXHR.status){
				...
				break;
				default:
					alertBox("HTTP status error: " + jqXHR.status + " - " + textStatus,"ext-mb-error");
...
function licenseSwapDo(){
	alertBox("Do the swap - not yet tested");
}
...
function load_private_note(el){
	...
	noteRequest.onerror = function() {
		alertBox("There was a connection error - unable to connect to target server.","ext-mb-error");
	};
			...
			switch (jqXHR.status){
				case 200:
					...
					if (data.error){
						...
						updateMessage = data.content.message;
						delete data.content.message; delete data.content.url; delete data.content.type; delete data.content.serial_in;
						document.getElementById("subsc_result").innerHTML = "<p class='success'>" + updateMessage + "</p><p><b>Results:</b><br />" + json_exploder(data.content) + "</p>";
					}
				break;
				default:
					alertBox("Invalid data sent to server - Error " + jqXHR.status + " received at " + API_PATH + "subscription/sync:" + $("#subsc_to_appliance_form").serialize(),"ext-mb-error");
...
function syncApplianceTab(serial) {
	url = "https://www.mattplatts.com/subsc/sync/update.cgi?sn=";
	url += serial + ""; 
...
function syncAppliance(){
	...
	url = "https://mattplatts.com/subsc/sync/update.cgi?sn=";
