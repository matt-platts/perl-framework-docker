...
#my $SESSIONS_FOLDER  = "/tmp/register_ra_sessions/";
#my $SESSIONS_TIMEOUT = 30;      #30 minute timeout for session files

sub Save_recursive($$$);

###########################################
###
### new (session_id, session_folder, session_timeout)
### session_id = identifier of session; if session is not found, a new ID is created
###
...
sub new($$$$) {
    my ($proto, $session_id, $session_folder, $session_timeout) = @_;
    my $self = {};
    my $class = ref($proto) || $proto;
    bless($self, $class);
    $self->{SESSIONS_FOLDER}  = $session_folder;
    $self->{SESSIONS_TIMEOUT} = $session_timeout;
