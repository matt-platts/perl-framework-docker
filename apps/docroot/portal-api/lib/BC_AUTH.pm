...
sub authenticate {
	...
	if ($debug_login){
		...
		print "ON SUB BC_AUTH::AUTHENTICATE";
	}
	my $username = shift;
	my $password = shift;
	...
	if ($CONFIG::limit_users){ # if we are using a limited user list
		...
		foreach my $key ( keys %CONFIG::valid_users){ # check valid users hash first before checking LDAP
			if ($key eq $username){
				...
				open (OUTF,">/apps/docroot/portal-api/log/debug_log.txt");
...
sub usage {
	...
	use SYMC_AUTH qw(authenticate);

	and call the authentication function using:

	BC_AUTH::authenticate(\$username,\$password);
EOT
}
